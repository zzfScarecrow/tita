...
const dist = function({ target, tplName }) {
  const templatePath = `template/crmRedux`
  const tplPath = `${target}/page/crm`
  const apiModulesPath = `${target}/src/api/modules`
  const widgetPath = `${target}/src/widget/${tplName}`
  return {
    output: [
      {
        tpl: resolve(`${templatePath}/crm.tpl`),
        realPath: `${tplPath}/${tplName}.tpl`
      },
      {
        tpl: resolve(`${templatePath}/crm.jsx`),
        realPath: `${widgetPath}/crm.jsx`
      },
      {
    ...
    insertEntry: function() {
      ...
      const insertion = [
        {
          key: `widget.${tplName}`,
          value: `./src/widget/${tplName}/crm.jsx`
        }
      ]
    ...
    insertPostProcessor: function() {
      ...
      const obj = {
        pagePath: `page/crm/${tplName}.tpl`,
        bundleName: `widget.${tplName}.js`,
        blockName: 'requireSrc',
        valid: true
      }
